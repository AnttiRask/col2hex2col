...
Thank you for your contributions to col2hex2col! Below is a checklist of
items to address before the PRs can be merged.
...
Decide on integration strategy with
...
**Rebase on current main branch** (critical - restores CRAN
instructions)
...
1.  Should out-of-range values be rejected with an error or clamped
    silently?
2.  Should vector recycling be supported in `*_to_hex()` functions?
3.  For PR \#5: Is this ready for review or should it be marked as
    draft?
4.  For achromatic colors in OKLCH: Should hue be set to 0 or left as
    computed?
